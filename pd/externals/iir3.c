...
static t_class *iir3_tilde_class;

typedef struct iir3_tilde {
  t_object self;
...
t_int *iir3_tilde_perform(t_int *w) {
  t_iir3_tilde *self = (t_iir3_tilde *)(w[1]);
  ...
  self->coefficients[5] = self->a1;
  self->coefficients[6] = self->a2;
  self->coefficients[7] = self->a3;
...
void iir3_tilde_dsp(t_iir3_tilde *self, t_signal **sp) {
  dsp_add(iir3_tilde_perform, DSP_ARG_COUNT, self, sp[0]->s_vec, sp[1]->s_vec,
          sp[0]->s_n);
}

void *iir3_tilde_new(t_symbol *s, int argc, t_atom *argv) {
  t_iir3_tilde *self = (t_iir3_tilde *)pd_new(iir3_tilde_class);
...
void iir3_tilde_free(t_iir3_tilde *self) {
  inlet_free(self->in);
...
void iir3_tilde_setup(void) {
  iir3_tilde_class = class_new(gensym("iir3~"), (t_newmethod)iir3_tilde_new,
                               (t_method)iir3_tilde_free, sizeof(t_iir3_tilde),
                               CLASS_DEFAULT, A_GIMME, 0);

  class_addmethod(iir3_tilde_class, (t_method)iir3_tilde_dsp, gensym("dsp"),
                  A_CANT, 0);
}
